...
namespace OOP_laba4
{
    //Класс StorageNode является узлом списка, который содержит ссылки на предыдущий и следующий узлы и значение элемента типа T.
    ...
    public class Storage<T> : IObservable
    {
        ...
        public StorageNode<T> GetFirst()
        {
            return first;
        }

        public StorageNode<T> First(StorageNode<T> first)
        {
            current = first;
            return first;
        }
        ...
        {
            var iter = CreateIterator();//Метод CreateIterator возвращает объект итератора типа StorageIterator<T>, который можно использовать для последовательного обхода элементов списка.
        ...
        public void Replace(T oldItem, T newItem)//заменяет один элемент списка на другой.
        {
        ...
        public void AddObserver(IObserver observer)//добавляет новый объект-наблюдатель в список и регистрирует данный класс в качестве одного из его субъектов (объектов, за которыми он следит)
        {
            observers.Add(observer);
            observer.GetSubjects().Add(this);
        }

        public void RemoveObserver(IObserver observer)//удаляет объект-наблюдатель из списка и отменяет регистрацию данного класса в качестве субъекта для данного наблюдателя
        {
            observers.Remove(observer);
            observer.GetSubjects().Remove(this);
        }

        public void NotifyAll(string property = null, object[] args = null)//уведомляет всех зарегистрированных наблюдателей об изменениях в данном объекте
        {
        ...
        public bool isObserver(IObserver observer)//проверяет, зарегистрирован ли данный объект-наблюдатель в списке
        {
            return observers.Contains(observer);
        }

        public void RemoveAllObserver()//удаляет всех зарегистрированных наблюдателей из списка
        {
            observers.Clear();
        }
        public List<IObserver> GetObservers()//возвращает список всех зарегистрированных наблюдателей
        {
            return observers;
        }
    }
